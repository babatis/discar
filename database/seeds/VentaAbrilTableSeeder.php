...
class VentaAbrilTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        DB::table('ventas_dtes')->delete();

        $path = public_path() . '/storage/xml/DTE_76944108_33_ABRIL.xml';
            ...
            foreach ($SetDTE as $dte) {
                ...
                if ($dte->Documento->Encabezado->IdDoc->Folio == $exam->Folio) {
                    ...
                    foreach ($dte->Documento->Encabezado as  $value) {
                        ...
                        $client_id = $client->where('rut', (string) $value->Receptor->RUTRecep)
                                            ->value('id');
                        
                        print('Cliente-id -> '.  $client_id);
                        $ventas->cliente_id = (string) $client_id;
                    ...
                    foreach ($dte->Documento->Detalle as  $value) {
                        ...
                        if (is_null($prod_id)) {
                            ...
                            $dtl->codinterno = strtoupper((string) $pd_id);
                            $dtl->nombre = $value->NmbItem;
                            $prod_costo = $prodVenta->where('prod_id', (string) $prod_id->id)->first();
